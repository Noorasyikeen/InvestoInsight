...
best_model_path = '/Users/zoeyneo/code/zoeyneo/investoinsight/epoch=7-step=3104.ckpt'
best_tft = TemporalFusionTransformer.load_from_checkpoint(best_model_path)

def load_pretrained_model():
    return best_tft

def make_predictions(best_tft, st_ticker):
    # if __name__ == '__main__':
        ticker = st_ticker
        df = pd.read_csv('/Users/zoeyneo/code/zoeyneo/investoinsight/sorted_feature_matrix.csv')
        ...
        encoder_data = df_pytorch[lambda x: (x.Tickers == ticker) & (x.Index > x.Index.max() - 12)]
        # select last known data point and create decoder data from it by repeating it and incrementing the month
        last_data = df_pytorch[lambda x: (x.Tickers == ticker) & (x.Index == x.Index.max())]
